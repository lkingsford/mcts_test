...
def test_back_propogate_one_player():
    ...
    root = Node(0, -1, None, 0, permitted_actions=[2], next_player=0)
    root.expansion((lambda action, state: sample_act([2], action, state)))
...
def test_back_propogate_multiple_player():
    ...
    root = Node(0, -1, None, 1, permitted_actions=[1], next_player=1)
...
def test_play_out():
    ...
    root = Node(0, -1, None, 1, permitted_actions=[1, 2, 3], next_player=1)
...
def test_expansion_fully_explored():
    ...
    root = Node(0, -1, None, 8, permitted_actions=[1, 2, 3], next_player=1)
    root.expansion(
        default_sample_act,
    )
...
def test_selection_unvisited():
    ...
    root = Node(0, -1, None, 0, permitted_actions=[1, 2], next_player=1)
    root.expansion((lambda action, state: sample_act([1, 2], action, state)))
